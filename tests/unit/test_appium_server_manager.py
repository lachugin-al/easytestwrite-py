...
# Context keys that we will automatically include in logs
...
def _file_sink_processor(
    ...
    # Prepare the JSON line in advance so we write the same content to both files
    ...
    try:
        ...
        # Never fail due to log writing problems
        pass
...
def current_test_log_path(test_name: str | None = None) -> Path:
    """Return the path to the current test's log file (or expected test) if its name is known."""
    _ensure_log_dir()
    # If the test name is not provided, return the global log file
    if not test_name:
...
def bind_context(
    *, settings: Any | None = None, driver: Any | None = None, test_name: str | None = None
    ...
    """Bind platform/device/test/session info to the logging context."""
    platform = None
    device = None
    session_id = None
...
def setup_logging() -> None:
    """
    Centralized configuration of structured logging with JSON output and file duplication.

    Includes:
    - Log level from MOBIAUTO_LOG_LEVEL
    - ISO 8601 timestamp (key: timestamp)
    - Context (platform, device, test, session_id) via contextvars
    - Duplication of each record into artifacts/logs/framework.log and test_<name>.log
    - Unified JSON format printed to stdout (compatible with current unit tests)
    """
    import logging
    ...
    structlog.configure(
        processors=[
            ...
            _drop_none_values,
            _file_sink_processor,  # Duplicate the record into file(s)
    ...
    # Sync log level with stdlib logging (for third-party libs)
...
def get_logger(name: str | None = None) -> Any:
    # Ensure logging is configured even in bare unit tests without the plugin
